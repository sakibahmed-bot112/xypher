module.exports = {
  ...
  onStart: async function ({ args, threadsData, message, event, api, getLang, envCommands }) {
    ...
    // ---------- Starry background: colored twinkling stars ----------
    const drawStarryBackground = (ctx, width, height, starCount = 250) => {
      // fill black
      ctx.fillStyle = "#000000";
      ctx.fillRect(0, 0, width, height);

      const starColors = ["#ffffff", "#ffe9c4", "#d4fbff", "#ffd700", "#ff69b4", "#87ceeb", "#00ffcc", "#a8ffb0", "#ffd7a8"];

      // small stars
      for (let i = 0; i < starCount; i++) {
        const x = Math.random() * width;
        const y = Math.random() * height;
        const radius = Math.random() * 1.6 + 0.2; // size variation
        const color = starColors[Math.floor(Math.random() * starColors.length)];
        const alpha = Math.random() * 0.9 + 0.1;
        const glow = Math.random() * 20 + 4;
      ...
      const twinkleCount = Math.max(4, Math.floor(starCount * 0.02));
      for (let t = 0; t < twinkleCount; t++) {
        const x = Math.random() * width;
        const y = Math.random() * height;
        ...
        ctx.shadowColor = color;
        ctx.shadowBlur = 30 + Math.random() * 40;
        ctx.fill();
        ctx.closePath();
      }
      ...
      for (let n = 0; n < 6; n++) {
        const x = Math.random() * width;
        const y = Math.random() * height;
        const maxR = Math.max(width, height) * (0.06 + Math.random() * 0.12);
        const grd = ctx.createRadialGradient(x, y, 0, x, y, maxR);
        const hue = Math.floor(Math.random() * 360);
        grd.addColorStop(0, `rgba(255,255,255,${0.02 + Math.random() * 0.08})`);
    ...
    const getAvatar = async (uid, name) => {
      try {
        ...
        console.error("Failed to fetch avatar for", uid, error);
        ...
        ctx.fillText((name || "U").charAt(0).toUpperCase(), 256, 256);
    ...
    if (args[0]?.toLowerCase() === 'all') {
      ...
      // Draw starry background (colored, twinkling)
      drawStarryBackground(ctx, 1200, 1700, 360);

      // faint grid lines for style (as original)
      ...
      for (const i of rankOrder) {
        ...
        ctx.textAlign = 'center';
        ctx.font = `bold ${Math.floor(pos.r * 0.3)}px "BeVietnamPro", "sans-serif"`;
        ctx.fillStyle = '#FFFFFF';
        ctx.fillText(fitText(ctx, user.name, pos.r * 2.2), pos.x, pos.y + pos.r + 40);
        ctx.font = `normal ${Math.floor(pos.r * 0.25)}px "BeVietnamPro", "sans-serif"`;
      ...
      for (const uid of targetUsers) {
        ...
        // starry background for user card
        drawStarryBackground(ctx, 800, 1200, 220);
