module.exports = { config: { name: "count", version: "2.5", // Updated version author: "Mahi--", countDown: 10, role: 0, description: { vi: "Xem bảng xếp hạng tin nhắn dưới dạng ảnh (từ lúc bot vào nhóm).", en: "View the message count leaderboard as an image (since the bot joined the group)." }, category: "box chat", guide: { vi: "   {pn}: Xem thẻ hoạt động của bạn." + "\n   {pn} @tag: Xem thẻ hoạt động của người được tag." + "\n   {pn} all: Xem bảng xếp hạng của tất cả thành viên.", en: "   {pn}: View your activity card." + "\n   {pn} @tag: View the activity card of tagged users." + "\n   {pn} all: View the leaderboard of all members." }, envConfig: { "ACCESS_TOKEN": "6628568379%7Cc1e620fa708a1d5696fb991c1bde5662" } },
...
onStart: async function ({ args, threadsData, message, event, api, getLang, envCommands }) {
	...
	// --- NEW: space background helpers ---
	const drawStars = (ctx, width, height, count = 200) => {
	...
	const drawNebulaGradient = (ctx, width, height, color1, color2) => {
		const g = ctx.createRadialGradient(width * 0.2, height * 0.2, 10, width * 0.6, height * 0.4, Math.max(width, height));
	...
	const drawSpaceBackground = async (ctx, width, height) => {
		// try to use a local space image if present in assets/count/space.jpg or space.png
		const { existsSync } = require('fs');
		const spacePaths = [resolve(__dirname, 'assets', 'count', 'space.jpg'), resolve(__dirname, 'assets', 'count', 'space.png')];
		for (const p of spacePaths) {
			if (existsSync(p)) {
				try {
					...
					ctx.fillRect(0, 0, width, height);
					// add stars on top to integrate
					drawStars(ctx, width, height, 120);
					return;
				} catch (e) {
					console.warn('Failed to draw local space image:', e);
				}
			}
		}

		// fallback: procedural space background
		...
		// add colored nebulas
		drawNebulaGradient(ctx, width, height, 'rgba(72,61,139,0.25)', 'rgba(0,0,0,0)');
		drawNebulaGradient(ctx, width, height, 'rgba(255,20,147,0.12)', 'rgba(0,0,0,0)');

		// add stars
		drawStars(ctx, width, height, Math.floor((width * height) / 8000));
	};
	// --- END space helpers ---
	...
	if (args[0]?.toLowerCase() === 'all') {
		...
		// draw space background instead of simple gradient
		await drawSpaceBackground(ctx, 1200, 1700);
		...
		for(const uid of targetUsers) {
			...
			// draw space background for user card
			await drawSpaceBackground(ctx, 800, 1200);
