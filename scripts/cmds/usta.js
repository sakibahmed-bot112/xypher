  ...
  onStart: async function ({ event, api, usersData }) {
    try {
      ...
      if (event.mentions && Object.keys(event.mentions).length > 0) {
        ...
        return api.sendMessage("- কাকে উস্তা মারবি মেনশন দে..!😼", event.threadID, event.messageID);
      }
      ...
      // বেস ইমেজ
      const baseImage = await Canvas.loadImage("https://files.catbox.moe/rt64cx.jpg");
      ...
      const stomachX = Math.round(canvas.width * 0.35);
      const stomachY = Math.round(canvas.height * 0.25);
      const chestX = Math.round(canvas.width * 0.65);
      const chestY = Math.round(canvas.height * 0.50);
      ...
      // সেভ + সেন্ড
      const path = __dirname + "/tmp/usta_" + Date.now() + ".png";
      await fs.ensureDir(__dirname + "/tmp");
      fs.writeFileSync(path, canvas.toBuffer());

      api.sendMessage(
        { body: ` ${senderName} উস্টা মারলো ${targetName} আবাল কে..!🦵`, attachment: fs.createReadStream(path) },
        event.threadID,
        () => fs.unlinkSync(path),
        event.messageID
      );
