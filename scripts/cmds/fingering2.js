module.exports = {
  ...
  onLoad: async function () {
    ...
    const path = resolve(dirMaterial, "fingeringv2.png");

    if (!existsSync(dirMaterial)) mkdirSync(dirMaterial, { recursive: true });
    if (!existsSync(path)) {
      await downloadFile("https://i.imgur.com/CQQZusa.jpeg", path);
    }
  },
  ...
  makeImage: async function ({ one, two }) {
    ...
    let baseImg = await jimp.read(__root + "/fingeringv2.png");
