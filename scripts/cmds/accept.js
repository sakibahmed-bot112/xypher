...
module.exports = {
  ...
  onReply: async function ({ Reply, event, api }) {
    ...
    if (args[0] === "add") {
      ...
      if (!isThreadAdmin && !isBotAdmin) {
        return api.sendMessage("🎀🐥 𝐎𝐧𝐥𝐲 𝐚𝐝𝐦𝐢𝐧 𝐜𝐚𝐧 𝐝𝐞𝐥𝐞𝐭𝐞 𝐟𝐫𝐢𝐞𝐧𝐝 𝐫𝐞𝐪𝐮𝐞𝐬𝐭𝐬", event.threadID, event.messageID);
      }
    ...
    for (const stt of targetIDs) {
      ...
      newTargetIDs.push(u);
      promiseFriends.push(api.httpPost("https://www.facebook.com/api/graphql/", currentForm));
    }
    ...
    for (let i = 0; i < newTargetIDs.length; i++) {
      try {
        ...
        if (JSON.parse(res).errors) {
          ...
          const timeFormatted = moment(u.time * 1000).tz("Asia/Manila").format("DD/MM/YYYY HH:mm:ss");
          success.push(`🎀🐥\n𝐍𝐚𝐦𝐞: ${u.node.name}\n𝐔𝐢𝐝: ${u.node.id}\n𝐓𝐢𝐦𝐞: ${timeFormatted}`);
          api.setMessageReaction("✅", event.messageReply?.messageID || event.messageID, () => {}, true);
        }
      } catch {
    ...
    let msg = "";
    if (success.length) msg += `🎀🐥\n𝐅𝐫𝐢𝐞𝐧𝐝 𝐫𝐞𝐪𝐮𝐞𝐬𝐭𝐬 𝐝𝐞𝐥𝐞𝐭𝐞𝐝 :\n\n${success.join("\n\n")}`;
    if (failed.length) msg += `\n\n⚠️ 𝐅𝐚𝐢𝐥𝐞𝐝:\n${failed.join("\n")}`;
    api.sendMessage(msg || "🎀🐥 𝐍𝐨 𝐫𝐞𝐪𝐮𝐞𝐬𝐭𝐬 𝐝𝐞𝐥𝐞𝐭𝐞𝐝 .", event.threadID, event.messageID);
  ...
  onStart: async function ({ event, api, args, commandName }) {
    ...
    try {
      ...
      const timeFormatted = moment(request.time * 1000).tz("Asia/Manila").format("DD/MM/YYYY HH:mm:ss");
